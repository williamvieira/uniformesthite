...
if (!nbd_is_product($product_id)) {
    echo sprintf('<p>%s, <a href="%s">%s</a></p>', __('No product has been selected', 'web-to-print-online-designer'), esc_url(home_url('/')), __('Back', 'web-to-print-online-designer'));
    die();
}
if (wp_is_mobile() && $hide_on_mobile == 'yes'):
    ...
    if (!nbd_check_permission()):
                ...
                if (nbdesigner_get_option('nbdesigner_share_design') == 'yes' && isset($_GET['nbd_share_id']) && $_GET['nbd_share_id'] != ''):
                    ...
                    if (isset($_GET['nbd_share_id']) && $_GET['nbd_share_id'] != '') {
                        $url = add_query_arg(
                            ...
                            'nbd_share_id' => $_GET['nbd_share_id']), getUrlPageNBD('create'));
                    }
                    ?>
        ...
        foreach ($settings as $key => $val):
            ...
            <?php endif; ?>
        <?php endforeach; ?>
        <?php $meta = unserialize(get_post_meta($product_id, '_designer_setting', true)) ?>
        <?php $colors = $meta[0]['area_design_ml_color'];
        ?>
                    var _colors = NBDESIGNCONFIG['nbdesigner_hex_names'].split(','), colorPalette = [], row = [];

        <?php if ($colors): ?>
            <?php foreach ($colors as $color): ?>
                            row.push('<?php echo $color; ?>');
            <?php endforeach; ?>
        <?php else: ?>
                ...
                <div style="width: 100%; height: 100%;" ng-controller="DesignerController" ng-cloak>
                    <div id="design-container" class="design-mode" ng-class="designMode == 'custom' ? 'active' : ''">
                        ...
                        <div id="od_config" ng-class="modeMobile ? 'mobile' : 'modepc'">	
                            ...
                            <!-- COMENTADO MULTLINKS <span class="hide-tool-config fa fa-chevron-down e-shadow e-hover-shadow item-config" ng-hide="modeMobile" ng-style="{'display' : (pop.text == 'block' || pop.art == 'block' || pop.qrcode == 'block' || pop.clipArt == 'block' || pop.draw == 'block') ? 'block' : 'none'}"></span> -->
