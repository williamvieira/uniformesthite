...
(function() {
    ...
    if (!mobile) jQuery("#layers").sortable({
        stop: function(event, ui) {
            ...
            if (scope.$root.$$phase !==
                "$apply" && scope.$root.$$phase !== "$digest") scope.$apply()
        }
    });
    ...
    canvas.on("path:created", function(e) {
        var scope = angular.element(document.getElementById("designer-controller")).scope(),
            index = scope.canvas.getObjects().length -
            1;
        scope.addLayers(scope.canvas.item(index), scope.canvas.item(index).get("type"), index);
        showLayer();
        if (scope.$root.$$phase !== "$apply" && scope.$root.$$phase !== "$digest") scope.$apply()
    });
    ...
    canvas.on("object:selected", function(options) {
        var scope =
            angular.element(document.getElementById("designer-controller")).scope();
        var object = options.target;
        ...
        if (object.type ===
            "i-text" || object.type === "text" || object.type === "curvedText") {
            ...
            if (_.size(scope._allFonts)) {
                ...
                if (!match) scope.currentFont = {
                    alias: "Arial",
                    name: "Arial"
                }
            } else scope.currentFont = {
                alias: "Arial",
                name: "Arial"
            };
        ...
        if (object.type ===
            "curvedText" && NBDESIGNCONFIG["lang_rtl"] == "rtl")
    ...
    canvas.on("object:rotating", function(options) {
        ...
        if (object.type === "path-group" || object.type ===
            "path") $("#rotation-svg").val(angle).trigger("change")
    });
    canvas.on("object:modified", function(options) {
        ...
        if (object.type === "i-text" || object.type ===
            "text" || object.type === "curvedText") {
        ...
        if (object.type === "image" || object.type === "custom-image") $("#rotation-image").val(angle).trigger("change");
        if (object.type === "rect" ||
            object.type === "triangle" || object.type === "line" || object.type === "polygon" || object.type === "circle") $("#rotation-shape").val(angle).trigger("change");
        if (object.type === "path-group" || object.type === "path") $("#rotation-svg").val(angle).trigger("change");
        scope.setUndoRedo({
            element: object,
            parameters: JSON.stringify(scope.currentItemParameters),
            interaction: "modify"
        });
        scope.$apply();
        return
        ...
        confirm: function() {
            ...
            if (index == null) {
                ...
                if (scope.canvas.getActiveGroup()) {
                    ...
                    scope.deactiveGroup()
                } else scope.canvas.remove(scope.canvas.getActiveObject());
                scope.editableItem = null;
                scope.closePopover()
            } else {
                ...
                layer_name.remove(scope.canvas.item(index).get('layerName'));
...
angular.module("app").controller("DesignerController", function($scope, PatternService, ProductService, localStorageService, FontService, ArtService, QrcodeService, $interval, $upload, $timeout, LanguageService, AdminTemplateService, CartDesignsService, UserDesignsService) {
    ...
    $scope.shadow.color = NBDESIGNCONFIG["nbdesigner_default_color"].substr(1,
        7);
    $scope.loaded = 0;
    $scope.zoom = 1;
    $scope.settings = {};
    ...
    $scope.colorBrush = NBDESIGNCONFIG["nbdesigner_default_color"].substr(1,
        7);
    ...
    $scope.orientationActiveRedoStatus =
        false;
    $scope.currentItemParameters = {};
    ...
    $scope.dropboxImages = [];
    if (NBDESIGNCONFIG["nbdesigner_show_all_color"] == "yes") $scope.backgroundPalette = ["#f44336", "#e91e63", "#9c27b0", "#673ab7", "#3f51b5", "#2196f3", "#03a9f4", "#00bcd4", "#009688", "#4caf50", "#8bc34a", "#cddc39", "#ffeb3b", "#ffc107", "#ff9800", "#ff5722", "#795548", "#9e9e9e", "#607d8b", "#374046"];
    if (NBDESIGNCONFIG["nbdesigner_show_all_color"] ==
        "no") $scope.backgroundPalette = _.uniq(_.flatten(colorPalette));
    $scope.changeDesignMode = function(mode) {
        $scope.designMode = mode;
        if (mode == "upload") $timeout(function() {
            if (jQuery("#nbd-upload-note").hasClass("first_time_in_hour")) jQuery("#nbd-upload-note").triggerHandler("click")
        }, 500);
    ...
    $scope.ajustAfterAddItem = function(type) {
        ...
        var layerName = layer_name[index];
        ...
        $scope.addLayers($scope.canvas.getActiveObject(),
            type, index)
    };
    ...
    $scope.getItemConfig = function(item, prop) {
        if (!!item.configuration) {
            var config = item.configuration;
            if (config.hasOwnProperty(prop)) return config[prop];
            else return false
        } else return false
    };
    $scope.addText = function() {
        ...
        $scope.showPopover("text");
        $scope.zoomCanvas();
        var text = new fabric.Text($scope.settings.nbdesigner_default_text, {
            left: 100,
            top: 10,
            ...
            if (isFontAvailable(name)) {
                $scope.setUndoRedo({
                    ...
                    parameters: JSON.stringify({
                        "fontFamily": $scope.canvas.item($scope.editableItem).fontFamily
                    }),
                    interaction: "modify"
                });
                $scope.currentFont = font;
    ...
    $scope.toggleItalic = function() {
        ...
        $scope.setUndoRedo({
            element: $scope.canvas.item($scope.editableItem),
            parameters: JSON.stringify({
                "fontStyle": $scope.canvas.item($scope.editableItem).fontStyle
            }),
            interaction: "modify"
        });
        if ($scope.editable.italic) {
            $scope.editable.italic =
                false;
    ...
    $scope.toggleTextCase = function() {
        if ($scope.editableItem == null) return;
        if (isUpperCase($scope.editable.text)) $scope.editable.text = $scope.editable.text.toLowerCase();
        else $scope.editable.text = $scope.editable.text.toUpperCase();
        $scope.canvas.item($scope.editableItem).setText($scope.editable.text);
        $scope.drawCanvas()
    };
    $scope.toggleBold =
        function() {
            ...
            $scope.setUndoRedo({
                element: $scope.canvas.item($scope.editableItem),
                parameters: JSON.stringify({
                    "fontWeight": $scope.canvas.item($scope.editableItem).fontWeight
                }),
                interaction: "modify"
            });
    ...
    $scope.toggleTextDecoration = function(decoration) {
        ...
        $scope.setUndoRedo({
            element: $scope.canvas.item($scope.editableItem),
            parameters: JSON.stringify({
                "textDecoration": $scope.canvas.item($scope.editableItem).textDecoration
            }),
            interaction: "modify"
        });
        if ($scope.editable.textDecoration == decoration) {
            ...
            $scope.editable.textDecoration = decoration;
    ...
    $scope.changeShadow =
        function() {
            ...
            if ($scope.shadow.color.charAt(0) == "r" || $scope.shadow.color.charAt(0) == "#") color = tinycolor($scope.shadow.color).toRgb();
            else color = hexToRGB("#" + $scope.shadow.color);
    ...
    $scope.flipHorizontal = function() {
        ...
        $scope.setUndoRedo({
            element: $scope.canvas.item($scope.editableItem),
            parameters: JSON.stringify({
                "flipY": $scope.canvas.item($scope.editableItem).flipY
            }),
            interaction: "modify"
        });
    ...
    $scope.flipVertical =
        function() {
            ...
            $scope.setUndoRedo({
                element: $scope.canvas.item($scope.editableItem),
                parameters: JSON.stringify({
                    "flipX": $scope.canvas.item($scope.editableItem).flipX
                }),
                interaction: "modify"
            });
    ...
    $scope.ShiftLeft = function() {
        ...
        $scope.setUndoRedo({
            element: $scope.canvas.item($scope.editableItem),
            parameters: JSON.stringify({
                "left": $scope.canvas.item($scope.editableItem).left
            }),
            interaction: "modify"
        });
    ...
    $scope.ShiftUp = function() {
        ...
        $scope.setUndoRedo({
            element: $scope.canvas.item($scope.editableItem),
            parameters: JSON.stringify({
                "top": $scope.canvas.item($scope.editableItem).top
            }),
            interaction: "modify"
        });
    ...
    $scope.align = function(layout) {
        ...
        $scope.setUndoRedo({
            element: $scope.canvas.item($scope.editableItem),
            parameters: JSON.stringify({
                "textAlign": $scope.canvas.item($scope.editableItem).textAlign
            }),
            interaction: "modify"
        });
    ...
    $scope.setBackground = function(e) {
        ...
        $scope.setUndoRedo({
            element: $scope.canvas.item($scope.editableItem),
            parameters: JSON.stringify({
                "textBackgroundColor": $scope.canvas.item($scope.editableItem).getTextBackgroundColor()
            }),
            interaction: "modify"
        });
        if (!e) {
        ...
        } else $scope.canvas.item($scope.editableItem).setTextBackgroundColor("");
        $scope.drawCanvas()
    };
    ...
    $scope.changeCurve = function(val) {
        ...
        var option = {};
        if ($scope.rtlLang) option.rtl = true;
        ...
        var index = $scope.canvas.getObjects().length - 1;
        if (NBDESIGNCONFIG["lang_rtl"] == "rtl") $scope.setItemConfig($scope.canvas.item(index), "rtl", true);
        $scope.canvas.setActiveObject($scope.canvas.item(index));
    ...
    $scope.changePatternRepeat = function(val) {
        $scope.canvas.item($scope.editableItem).fill.repeat = val;
        $scope.drawCanvas()
    };
    ...
    $scope.initStage = function(data) {
        ...
        if (attachProductInfo) {
            ...
            if (angular.isDefined(data.variation_id)) NBDESIGNCONFIG["variation_id"] =
                data.variation_id;
            if (angular.isDefined(data.product_info)) jQuery("#product-info-wrap-inner").html(data.product_info.html);
            attachProductInfo = 0
        }
        if (angular.isDefined(data.option)) {
            $scope.productOptions = data.option;
            if (NBDESIGNCONFIG["show_variation"] == "3") jQuery("#dg-load-product").modal("show");
        ...
        if (toType(data.design) == "object" && !$scope.keepLocalDesign) {
            ...
            var multisite = false;
            if (multisite) _.each($scope.canvases, function(canvas, side) {
                _.each(canvas["objects"], function(item, index) {
                    if (item.type == "image") {
                        var _src = item.src;
                        var src = _src.split("nbdesigner");
                        $scope.canvases[side]["objects"][index]["src"] = NBDESIGNCONFIG["nbd_content_url"] +
                            src[1]
                    }
                })
            });
            if (toType(data.ref) === "array") $scope.refProduct = data.ref;
            if (toType(data.config_ref) === "object") $scope.ajustmentScale = data.config_ref["scale"];
            ...
            if ($scope.currentProductId != "") var sid = "p_" + scope.currentProductId,
                cid = "c_" + scope.currentProductId;
            else var sid = "p_" + NBDESIGNCONFIG["product_id"],
            ...
            if (toType(localStorageService.get(sid)) === "object" && NBDESIGNCONFIG["task"] == "new" && !$scope.ignoreLocalDesign && !NBDESIGNCONFIG["is_designer"] && NBDESIGNCONFIG["nbdesigner_save_latest_design"] == "yes") {
                ...
                if (toType(data.config) === "object") $scope.ajustmentScale = data.config["scale"];
                if (toType(data.fonts) === "array") {
                    var font_url = NBDESIGNCONFIG["font_url"];
                    $scope.loadAdminDesignFont(data.fonts, font_url)
                }
    ...
    $scope.loadAdminListDesign = function() {
        if (!_.size($scope.adminListTemplate)) {
            ...
            AdminTemplateService.getDesign().then(function(data) {
                if (data["mes"] == "success") _.each(data.data, function(value, index) {
    ...
    $scope.loadDesignInCart = function(force) {
        if (!_.size($scope.listDesignsInCart) || force != undefined) {
            ...
            CartDesignsService.getDesign().then(function(data) {
                if (data["flag"] == 1) _.each(data.designs, function(value, index) {
    ...
    $scope.loadCartDesign = function($ref_id) {
        ...
        CartDesignsService.getDesign($ref_id).then(function(data) {
            if (data["flag"] == 1) {
                $scope.canvas.clear();
                if ($scope.flagChangeDimension) $scope.flagChangeOrientation = [];
                $scope.initStage(data);
    ...
    $scope.loadUserDesign = function(id) {
        ...
        if (!_.size($scope.listUserDesigns)) {
            ...
            UserDesignsService.getDesign(id).then(function(data) {
                ...
                } else if (data["flag"] == 2) $scope.settings["is_logged"] = 0;
                hide_indicator()
            })
        }
    };
    ...
    $scope.changeDimension = function(initReDesign) {
        ...
        _.each($scope.varaints.areaDesign, function(val, key) {
            ...
            if ($scope.customWidth > $scope.customHeight) {
                ...
                $scope.varaints.areaDesign[key]["img_src_width"] =
                    $scope.varaints.areaDesign[key]["area_design_width"] = 500;
                ...
                $scope.varaints.areaDesign[key]["img_src_left"] = $scope.varaints.areaDesign[key]["area_design_left"] = Math.round(250 - $scope.varaints.areaDesign[key]["area_design_width"] /
                    2)
            }
        });
        var len = $scope.currentVariant.info.length;
        if (len >= $scope.customSide) $scope.varaints.areaDesign = $scope.varaints.areaDesign.slice(0, $scope.customSide);
        else
        ...
        $scope.changeOrientation();
        if (!initReDesign) jQuery("#dg-custom-dimension").modal("hide")
    };
    ...
    $scope.insertPage = function() {
        ...
        if ($scope.pageSourceType == 1) $scope.varaints.areaDesign.splice(insertIndex, 0, $scope.currentPageSource);
        else {
            ...
            if ($scope.customWidth2 > $scope.customHeight2) {
                ...
                $scope.varaints.areaDesign[insertIndex]["img_src_left"] =
                    $scope.varaints.areaDesign[insertIndex]["area_design_left"] = 0;
                ...
                $scope.varaints.areaDesign[insertIndex]["img_src_height"] =
                ...
                $scope.varaints.areaDesign[insertIndex]["img_src_left"] = $scope.varaints.areaDesign[insertIndex]["area_design_left"] = Math.round(250 - $scope.varaints.areaDesign[insertIndex]["area_design_width"] /
                    2)
            }
        }
        ...
        for (var i = len; i > insertIndex; i--) {
            var _frame_src = "frame_" + (i - 1),
                _frame_dst = "frame_" + i;
            if (angular.isDefined($scope.svgs[_frame_src])) $scope.svgs[_frame_dst] = $scope.svgs[_frame_src];
            if (angular.isDefined($scope.canvases[_frame_src])) $scope.canvases[_frame_dst] = angular.copy($scope.canvases[_frame_src]);
            if (angular.isDefined($scope.dataCustomerDesign[_frame_src])) $scope.dataCustomerDesign[_frame_dst] = $scope.dataCustomerDesign[_frame_src]
        }
        $scope.flagCustomProduct =
            true;
        $scope.changeOrientation();
    ...
    $scope.loadAdminDesignFont = function(fonts, assets_font_url) {
        ...
        _.each(fonts, function(font) {
            if (font.type == "google") jQuery("head").append("<link href='https://fonts.googleapis.com/css?family=" + font.name.replace(" ", "+") + "' rel='stylesheet' type='text/css'>");
            else if (!isFontAvailable(font.alias)) {
                var font_url = font.url;
                if (!(font_url.indexOf("http") >
                        -1)) font_url = assets_font_url + font.url;
        ...
        var stop = $interval(function() {
            ...
            _.each(fonts, function(font, key) {
                if (!isFontAvailable(font.alias)) check[key] = false;
                else check[key] = true
            });
            _.each(check,
                function(c) {
    ...
    $scope.loadLanguage = function(code) {
        ...
        LanguageService.getLang(code).then(function(data) {
            if (data.mes == "success") {
                ...
                _.each($scope.langs, function(val,
                    key) {
    ...
    $scope.loadFont = function(index) {
        ...
        FontService.fn(function(data) {
            if (data.flag == "1") {
                ...
                if (data.google_font) $scope._allFonts = JSON.parse(JSON.stringify(data.google_font));
                else $scope._allFonts = [];
        ...
        ArtService.fn(function(data) {
            if (data.flag == "1") {
                ...
                if (toType(data.arts) == "array") {
                    ...
                    _.each($scope.arts,
                        function(art, key) {
                            if (!(art.url.indexOf("http") > -1)) $scope.arts[key]["url"] = NBDESIGNCONFIG["art_url"] + art.url
                        });
    ...
    $scope.countArtByCatID = function(input,
        cat_id) {
        var count = 0;
        _.each(input, function(val, key) {
            if (val.cat.length == 0) val.cat = ["0"];
    ...
    $scope.addArt = function(art) {
        if(art.title) layer_name.push(art.title);
        if (NBDESIGNCONFIG["is_wpml"]) art.url = art.url.replace(NBDESIGNCONFIG["home_url"], NBDESIGNCONFIG["icl_home_url"]);
        if (art.url.match(/\.(jpeg|jpg|gif|png)$/) != null) {
            ...
            $scope.canvas.add((new fabric.PathGroup(ob,
                op)).set({
                left: 100,
                top: 100
            }));
    ...
    $scope.updatePathColor = function(color) {
        ...
        if ($scope.editable.type == "path-group") $scope.editable._paths[color.key].fill =
            hex;
        $scope.drawCanvas()
    };
    ...
    $scope.changeOrientation = function(p) {
        ...
        if (!!$scope.currentVariant.info[p.id]["source"]["include_overlay"]) $scope.includeOverlay = parseInt($scope.currentVariant.info[p.id]["source"]["include_overlay"]);
        $scope.currentVariant.numberFrame = i;
        $scope.currentVariant.activeImages =
            _img;
        ...
        if ($scope.currentVariant.info[p.id]["source"]["show_overlay"] == "1" && $scope.currentVariant.info[p.id]["source"]["img_overlay"] != "") {
            ...
            fabric.Image.fromURL(overlay_src, function(op) {
                ...
                $scope.canvas.setOverlayImage(overlay_src, function() {
                    ...
                    $scope.canvas.overlayImage.scaleY =
        ...
        } else $scope.canvas.overlayImage = null;
        if (angular.isDefined($scope.canvases[p.name])) $scope.canvas.loadFromJSON($scope.canvases[p.name], function() {
            applyImageFilters();
            $scope.canvas.renderAll();
            $scope.updateCurrentLayerAfterDelete();
            if (angular.isUndefined($scope.flagChangeOrientation[p.id])) {
                ...
                $scope.flagChangeOrientation[p.id] =
                    1
            } else if ($scope.ignoreLocalDesign) {
                $scope.resizeDesign($scope.designScale, $scope.ajustmentScale);
                $scope.ignoreLocalDesign = false
            } else if ($scope.ajustmentOrientationScale[p.id] != $scope.designScale) $scope.resizeDesign($scope.designScale, $scope.ajustmentOrientationScale[p.id]);
            $scope.ajustmentOrientationScale[p.id] = $scope.designScale
        });
        if ($scope.bleedStatus) jQuery(".nbd-bleed, .nbd-safe-zone").show();
        else jQuery(".nbd-bleed, .nbd-safe-zone").hide();
        $scope.updateUndoRedoStatus();
        $scope.showAlignToolbar =
            false;
        $scope.customSide = $scope.currentVariant.info.length;
        $scope.zoom = 1;
        $scope.zoomCanvas();
    ...
    $scope.resizeDesign = function(newScale, oldScale) {
        ...
        if ($scope.currentVariant.info[frame_id]["source"]["show_overlay"] == "1" && $scope.currentVariant.info[frame_id]["source"]["img_overlay"] != "") {
            ...
            fabric.Image.fromURL(overlay_src, function(op) {
                width = op.width;
                height = op.height;
                $scope.canvas.setOverlayImage(overlay_src,
                    function() {
                        $scope.canvas.overlayImageLeft = 0;
                        $scope.canvas.overlayImageTop = 0;
                        $scope.canvas.overlayImage.scaleX = $scope.canvas.width / width;
    ...
    $scope.saveDesign = function(orientation) {
        ...
        $scope.zoomCanvas();
        $scope.drawCanvas();
        var json = canvas.toJSON(["itemId", "selectable", "lockMovementX", "lockMovementY", "lockScalingX", "lockScalingY",
            "lockRotation", "rtl", "elementUpload", "forceLock", "isBg", "layerName"
        ]);
        var config = {};
        if ($scope.modeMobile) {
            var _width = $scope.currentVariant.designArea.area_design_width * $scope.designScale,
        ...
        config["font_used"] =
            $scope.filterFontUsed();
        if ($scope.flagChangeDimension) config["custom_dimension"] = {
            width: $scope.customWidth,
    ...
    $scope.getUsedFonts = function() {
        ...
        $scope.canvas.forEachObject(function(object) {
            if (object.type ===
                "i-text" || object.type === "text" || object.type === "curvedText" || object.type === "textbox") fonts.push(object.getFontFamily())
        });
        return fonts
    };
    $scope.filterFontUsed = function() {
        ...
        _.forEach($scope.fontCollection, function(used_fonts) {
            _.each(used_fonts, function(used_font) {
                _.each($scope._allFonts, function(font) {
                    if (font.alias == used_font) usedFonts.push(font)
                })
            })
        });
    ...
    $scope.storeDesign = function() {
        ...
        var fonts = $scope.filterFontUsed(),
            config = {
                "scale": $scope.designScale,
                "dpi": $scope.productOptions.dpi,
                "only_svg": $scope.only_svg
            };
        if ($scope.flagChangeDimension) {
            ...
            if ($scope.initNoPage < $scope.varaints.areaDesign.length)
                if (angular.isDefined($scope.productOptions.price_per_page)) _customPrice += ($scope.varaints.areaDesign.length - $scope.initNoPage) * $scope.productOptions.price_per_page;
            config.initNoPage = $scope.initNoPage;
            config.custom_dimension = {
                width: $scope.customWidth,
                height: $scope.customHeight,
                price: _customPrice,
                side: $scope.customSide
            }
        }
        config["product"] = $scope.varaints.areaDesign;
    ...
    $scope.showBleed = function() {
        if ($scope.bleedStatus) jQuery(".nbd-bleed, .nbd-safe-zone").hide();
        else jQuery(".nbd-bleed, .nbd-safe-zone").show();
        $scope.bleedStatus = !$scope.bleedStatus;
        var ignore = localStorageService.get("nbd_ignore_mes");
        if (!ignore || ignore == 2) jQuery("#dg-bleed-tip").modal("show")
    };
    $scope.ignoreMessage = function() {
        if (jQuery("#nbd-bleed-notify").is(":checked")) localStorageService.set("nbd_ignore_mes", 1);
        else localStorageService.set("nbd_ignore_mes", 2)
    };
    $scope.zoomViewport = function() {
        $scope.designerWidth = $scope.originDesignerWidth * $scope.zoom;
        $scope.designerHeight = $scope.originDesignerHeight *
            $scope.zoom;
        var w = $(window).width();
    ...
    $scope.calcBleedSize = function(command, size, bleed, ratio) {
        ...
        switch (command) {
            ...
            case "left":
            case "top":
                result = (size + Math.round(bleed *
                    ratio)) * $scope.zoom * $scope.designScale;
                break
        }
        return result
    };
    ...
    $scope.addLayers =
        function(object, type, index) {
            ...
            if (type === "triangle") layer = {
                ...
                "title": object.layerName,
                "type": "triangle",
                "src": NBDESIGNCONFIG["assets_url"] + "images/mark/mark_13.svg",
                "index": index
            };
            if (type === "line") layer = {
            ...
            if (!object.get("selectable") || !object.get("visible")) layer["class"] = "lock";
            else layer["class"] = "";
    ...
    $scope.setStackPosition = function(command) {
        ...
        if (bgItemid && idActiveObject == 0) object.moveTo(1);
    ...
    $scope.updateCurrentLayerAfterDelete = function() {
        ...
        for (var i = 0; i < index; i++) {
            var type =
                $scope.canvas.item(i).get("type");
            $scope.addLayers($scope.canvas.item(i), type, i);
            if(!layer_name.contains($scope.canvas.item(i).get('layerName'))){
            layer_name.push($scope.canvas.item(i).get('layerName'));
        }
        }
    };
    ...
    $scope.getQrCode = function() {
        var type_qrcode = "svg";
        if (type_qrcode == "png") QrcodeService.fn($scope.qrCodeContent, function(data) {
            if (data.flag == "1") {
                ...
                if (NBDESIGNCONFIG["nbdesigner_cache_uploaded_image"] == "yes") localStorageService.set($scope.iid, $scope.uploadURL)
    ...
    $scope.setDrawingLineShadowWidth = function(value) {
        if ($scope.canvas.freeDrawingBrush) {
            var blur =
                parseInt(value, 10) || 0;
            if (blur > 0) $scope.canvas.freeDrawingBrush.shadow = new fabric.Shadow({
                blur: blur,
                offsetX: 10,
                offsetY: 10
            });
    ...
    $scope.getRandomColor = function() {
        return colorTrend[Math.floor(Math.random() *
            colorTrend.length)]
    };
    ...
    $scope.getBackgroundLayer = function() {
        ...
        $scope.canvas.forEachObject(function(obj) {
            if (obj.type ===
                "rect" && obj.isBg != undefined) itemId = obj.itemId
        });
        return itemId
    };
    ...
    $scope.addCircle = function() {
        $("#opacity_shape").slider("value", 0);
        var _default = $scope.designerWidth >
            50 ? 50 : $scope.designerWidth;
    ...
    $scope.setShapeColor = function(e) {
        ...
        if ($scope.editable.type === "line") $scope.changeStroke(e);
        else if ($scope.editable.type === "rect" || $scope.editable.type ===
            "triangle" || $scope.editable.type === "polygon" || $scope.editable.type === "circle") $scope.changeColor(e)
    };
    $scope.onFileUploadSelect = function($files) {
        ...
        } else $scope.fileUpload = $files
    };
    $scope.listFileUpload = [];
    $scope.fileUpload = [];
    $scope.startUploadDesign =
        function() {
            ...
            $scope.upload = $upload.upload({
                ...
                $scope.loading = false;
                if (data.flag == 1) $scope.listFileUpload.push({
                    src: data.src,
                    name: data.name
                });
                else alert(data.mes);
    ...
    $scope.completeUpload = function() {
        ...
        } else if ((NBDESIGNCONFIG["task2"] == "update" || NBDESIGNCONFIG["task"] == "new") && NBDESIGNCONFIG["ui_mode"] == 2) $scope.saveCart();
        else {
    ...
    $scope.onFileSelect = function($files) {
        ...
        for (var i = 0; i < $files.length; i++) {
            ...
            if ($files[i].size > max_size * 1024 * 1024) {
                $scope.errorUpload = langjs["MES_ERROR_MAXSIZE_UPLOAD"] + " " + max_size +
                    " MB";
                continue
    ...
    $scope.ajaxUploadFiles = function(i) {
        ...
        $scope.upload = $upload.upload({
            ...
            if (angular.isUndefined(data.src)) alert(data.mes);
            else {
                $scope.uploadURL.push(data.src);
                if (NBDESIGNCONFIG["nbdesigner_cache_uploaded_image"] == "yes") localStorageService.set($scope.iid, $scope.uploadURL)
            }
            if (i < $scope.file.length - 1) $scope.ajaxUploadFiles(i + 1);
    ...
    $scope.loadLocalStorageImage = function() {
        ...
        if (!!localSource && toType(localSource) == "array") $scope.uploadURL =
            localSource;
        $scope.loaded = 30
    };
    ...
    $scope.authenticateInstagram = function() {
        ...
        if (instaAccessToken) $scope.loadInstagramPhotos(instaAccessToken);
        else $scope._authenticateInstagram()
    };
    ...
    $scope._authenticateInstagram = function() {
        var popupLeft = (window.screen.width -
                700) / 2,
                ...
                if (data.data) {
                    ...
                    $.each(data.data, function(i, item) {
                        if (item.type == "image") {
                            ...
                            html += '<img class="img-responsive img-thumbnail nbdesigner_upload_image shadow hover-shadow" data-url="" onclick="NBDESIGNERFUNC.addFacebookImage(\'' +
    ...
    $scope.addFacebookImage = function(img) {
        var url = angular.isDefined(img.link) ? img.link : img;
        NBDESIGNERFUNC.addFacebookImage(url)
    };
    $scope.authenticateDropbox = function() {
        var dbtaAccessToken = localStorageService.get("nbd_dropbox_token");
        if (dbtaAccessToken) $scope.loadDropboxPhotos(dbtaAccessToken);
        else $scope._authenticateDropbox()
    };
    var _dropboxImages = [];
        ...
        function(url, params) {
            ...
            fabric.Image.fromURL(url, function(op) {
                ...
                if (params) {
                    ...
                    $scope.canvas.add(op);
                    $scope.showPopover("art");
                    $scope.canvas.setActiveObject($scope.canvas.item($scope.canvas.getObjects().length -
                        1));
                    var max_width = $scope.canvas.width * .9;
                    var max_height = $scope.canvas.height * .9;
                    var new_width = max_width;
    ...
    $scope.initCropCanvas = function(type) {
        ...
        if ($scope.canvas.item($scope.editableItem).get("type") ==
            "custom-image") _url = $scope.canvas.item($scope.editableItem).origin_src;
        var url = _url,
            _type = $scope.canvas.item($scope.editableItem).type;
        if (url == "") return;
        if (!$scope.modeMobile) {
            width = 600;
            height = 400
        ...
        originImg.onload = function() {
            ...
            canvas2.item(2)["setControlVisible"]("tl",
                false);
            canvas2.item(2)["setControlVisible"]("bl", false);
    ...
    $scope.cropImage = function() {
        ...
        img.onload = function() {
            ...
            var max_width = $scope.canvas.width *
                .7;
    ...
    $scope.toggleLockLayer = function(layer) {
        ...
        if ($scope.task !== "admindesign") {
            if (lock) return
        } else $scope.canvas.item(layer.index).set({
            "forceLock": !lock
        });
    ...
    $scope.duplicateItem = function() {
        ...
        if (!klass.async) $scope.canvas.add(klass.fromObject(json));
        else klass.fromObject(json, function(item) {
    ...
    $scope.setUndoRedo = function(undo, redo) {
        if (undo) {
            ...
            if ($scope.undos[$scope.currentVariant.orientationActive].length > 20) $scope.undos[$scope.currentVariant.orientationActive].shift()
        }
        if (redo) {
    ...
    $scope.redoDesign = function() {
        if ($scope.redos[$scope.currentVariant.orientationActive].length > 0) {
            ...
            if (last.interaction === "remove") {
                ...
                _parameters = JSON.stringify(parameters);
                $scope.setItemParameters(last.parameters,
                    last.element.itemId)
            }
            $scope.setUndoRedo({
                element: last.element,
    ...
    $scope.setItemParameters = function(parameters, itemId) {
        ...
        if (typeof params.src !== "undefined") fabric.Image.fromURL(params.src, function(op) {
            item.getElement().setAttribute("src",
                params.src);
            $scope.updateCurrentLayerAfterDelete();
            setTimeout(function() {
                $scope.drawCanvas()
    ...
    $scope.updateUndoRedoStatus = function() {
        ...
        if (angular.isDefined($scope.undos[$scope.currentVariant.orientationActive]))
            if ($scope.undos[$scope.currentVariant.orientationActive].length > 0) $scope.orientationActiveUndoStatus = true;
        if (angular.isDefined($scope.redos[$scope.currentVariant.orientationActive]))
            if ($scope.redos[$scope.currentVariant.orientationActive].length >
                0) $scope.orientationActiveRedoStatus = true
    };
    ...
    $scope.setElementUpload = function() {
        ...
        var object = $scope.canvas.item($scope.editableItem);
        if (object.type === "image" || object.type === "custom-image") {
            if (typeof object.elementUpload === "undefined") object.set({
                "elementUpload": true
            });
            else {
                var hasUpload = !object.elementUpload;
                object.set({
                    "elementUpload": hasUpload
                })
            }
            $scope.updatePositionReplaceButton()
        }
    ...
    $scope.init = function() {
        ...
        $scope.resetCustomDimension();
        $scope.initParams();
        if (NBDESIGNCONFIG["task"] == "reup" || NBDESIGNCONFIG["enable_upload_without_design"] ==
            2) $scope.designMode = "upload";
        if (!checkNavigator()) {
            hide_indicator();
            return
        }
        ...
        initTooltip();
        initDialogLang();
        if (NBDESIGNCONFIG["nbdesigner_dimensions_unit"] ==
            "mm") $scope.rateConvertCm2Px96dpi = $scope.rateConvertCm2Px96dpi * .1;
        else if (NBDESIGNCONFIG["nbdesigner_dimensions_unit"] == "in") $scope.rateConvertCm2Px96dpi = $scope.rateConvertCm2Px96dpi * 2.54;
        $scope.initStage(NBDESIGNCONFIG["product_data"]);
        if (angular.isUndefined(NBDESIGNCONFIG["product_data"].config) || angular.isUndefined(NBDESIGNCONFIG["product_data"].config.initNoPage)) $scope.initNoPage = NBDESIGNCONFIG["product_data"].product.length;
        else $scope.initNoPage = NBDESIGNCONFIG["product_data"].config.initNoPage;
        if (toType(NBDESIGNCONFIG["list_file_upload"]) == "array") $scope.listFileUpload = NBDESIGNCONFIG["list_file_upload"];
        $scope.initBrushes();
    ...
    $scope.debug =
        function() {
            var length = $scope.canvas.getObjects().length;
            var last_item = $scope.canvas.item(length - 1);
            console.log(last_item);
            console.log(last_item.getBoundingRect())
        };
    ...
    $scope.changeVariation = function() {
        ...
        if (variation_id == "") alert(langjs["PLEASE_CHOOSE_ALL_OPTION"])
    };
    $scope.cancelChangeVariation = function() {
        jQuery("#dg-product-info").modal("hide")
    };
    $scope.loadProductDescription = function($event,
        product) {
        ...
        ProductService.getProductDescription(product.product_id).then(function(data) {
            ...
            } else alert("Opps! Try again later!")
        })
    };
    $scope.state = "pub";
    window.scope = $scope
...
var NBDESIGNERCART = {
    build_form: function(data_img, data_json, fonts, config, action, svgs) {
        var design_blob = new Blob([JSON.stringify(data_json)], {
            ...
            config_blob = new Blob([JSON.stringify(config)], {
                type: "application/json"
            }),
            font_blob = new Blob([JSON.stringify(fonts)], {
                type: "application/json"
            });
        var fd = new FormData;
...
angular.module("app").filter("reverse", function() {
    ...
    function() {
        return function(input, name) {
            ...
            var output = [];
            _.each(input, function(val, key) {
                if (val.name.indexOf(name) > -1) output.push(val)
            });
            return output
        }
...
jQuery(document).ready(function($) {
    nbd_window.jQuery(".variations_form.cart").on("change", function() {
        if (NBDESIGNCONFIG["ui_mode"] == 1) {
            ...
            if (variation_id != "") {
                ...
                if (NBDESIGNCONFIG["nbdesigner_hide_button_cart_in_detail_page"] == "yes") nbd_window.jQuery('button[type="submit"].single_add_to_cart_button').hide()
            }
            NBDESIGNCONFIG["variation_id"] =
                variation_id;
            if (!scope.onLoadProduct && variation_id != "") {
                scope.loaded = 0;
                scope.onLoadProduct = true;
    ...
    nbd_window.jQuery(".reset_variations").on("click", function() {
        if (NBDESIGNCONFIG["ui_mode"] == 1) {
            var scope = angular.element(document.getElementById("designer-controller")).scope();
            nbd_window.NBDESIGNERPRODUCT.nbdesigner_unready();
            if (NBDESIGNCONFIG["nbdesigner_hide_button_cart_in_detail_page"] ==
                "yes") nbd_window.jQuery('button[type="submit"].single_add_to_cart_button').hide();
    ...
    $(document).bind("keydown", function(e) {
        var scope = angular.element(document.getElementById("designer-controller")).scope();
        //var object = scope.canvas.getActiveObject();
        var object = scope.editableItem;
        var $target = $(e.target);
        if (e.shiftKey) scope.holdShiftKey = true;
        else scope.holdShiftKey = false;
        switch (e.which) {
            case 37:
                if (object == null || $target.is('textarea,input[type="text"],input[type="number"]')) return;
                if (!object.get("lockMovementX")) scope.ShiftLeft();
                break;
            case 39:
                if (object == null || $target.is('textarea,input[type="text"],input[type="number"]')) return;
                if (!object.get("lockMovementX")) scope.ShiftRight();
                break;
            case 38:
                if (object == null || $target.is('textarea,input[type="text"],input[type="number"]')) return;
                if (!object.get("lockMovementY")) scope.ShiftUp();
                break;
            case 40:
                if (object == null || $target.is('textarea,input[type="text"],input[type="number"]')) return;
                if (!object.get("lockMovementY")) scope.ShiftDown();
                break;
            case 46:
                if (object == null || $target.is('textarea,input[type="text"],input[type="number"]')) return;
            ...
            case 67:
                if (object == null || $target.is('textarea,input[type="text"],input[type="number"]')) return;
                if (e.ctrlKey) scope.duplicateItem();
                break;
            case 90:
                if (e.ctrlKey) scope.undoDesign();
                break;
            case 89:
                if (e.ctrlKey) scope.redoDesign();
                break;
            case 107:
                if (object == null || $target.is('textarea,input[type="text"],input[type="number"]')) return;
                if (e.shiftKey) scope.scaleItem("+");
                break;
            case 109:
                if (object == null || $target.is('textarea,input[type="text"],input[type="number"]')) return;
                if (e.shiftKey) scope.scaleItem("-");
